<template>
    <div style="margin-left:auto;margin-right:auto;width:1000px;margin-top:-10px;">
        <div v-for="item in booklist" :key="item.id" style="margin-bottom: 0;width: 1000px;">
...
export default {
    ...
    computed: {
        ...mapGetters(['booklist']),
        book() {
            // return this.booklist.filter(book => book.language === "中文");
            return this.booklist.filter(book => {
                return (this.filters.language === this.$route.params.classifyitem || book.language === this.filters.language) &&
                    (this.filters.category === this.$route.params.classifyitem || book.category === this.filters.category);
            })
        }
    },
