<template>
    <div style="margin-left:auto;margin-right:auto;width:1000px;margin-top:-10px;">
        <div v-for="item in booklist" :key="item.id" style="margin-bottom: 0;width: 1000px;" v-if="booklist = false">
...
export default {
    ...
    computed: {
        ...mapGetters(['booklist']),
        book() {
            return this.booklistUpdate.find(book => book.name === this.$route.params.name);
        }
    },
