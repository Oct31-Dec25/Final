...
export default {
    ...
    computed: {
        ...mapGetters(['booklist']),
        book() {
            // return this.booklist.filter[(book => book.language === this.$route.params.classifyitem) || (book => book.publicationYear === this.$route.params.classifyitem)];
            const classifyitem = this.$route.params.classifyitem;
            return this.booklist.filter(book => {
                return book.language === classifyitem || book.publicationYear === Number(classifyitem)|| book.;
            });
        }
    },
