<template>
    <div style="margin-left:auto;margin-right:auto;width:1000px;margin-top:-10px;">
        <div v-for="item in book" :key="item.id" style="margin-bottom: 0;width: 1010px;">
...
export default {
    ...
    computed: {
        ...
        book() {
            //获取来自同组件的路由参数
            //（可能是语言"language",也可能是出版年份"publicationYear",还有可能是分类"Categories"）
            const classifyitem = this.$route.params.classifyitem;
            //通过获取路由参数,在booklist中过滤筛选并返回
            return this.booklist.filter(book => {
                return book.language === classifyitem || book.publicationYear === Number(classifyitem) || book.Categories === classifyitem;
            });
        }
    },
